...
public abstract class GuiVoxelBoxSettingsPanel extends AdvancedDrawGui implements ConfigPanel
{
	...
	{
		super.mouseMovedOrUp(mouseX + PANEL_LEFT, mouseY, mouseButton);
	}

	@Override
	public void mouseMoved(ConfigPanelHost host, int mouseX, int mouseY)
	{
		super.mouseMovedOrUp(mouseX + PANEL_LEFT, mouseY, -1);
	}
	...
	public void keyPressed(ConfigPanelHost host, char keyChar, int keyCode)
	{
		...
		if (keyCode == Keyboard.KEY_TAB)
		{
			...
			for (VoxelProperty<?> property : this.properties)
			{
				if (property.isFocusable() && next == null) next = property;
			...
			// If didn't find a focusable property after focused in the chain, use the one nearest the start
			if (next == null) next = before;
			
			// If we ARE focused and have a focusable property to switch to, then switch

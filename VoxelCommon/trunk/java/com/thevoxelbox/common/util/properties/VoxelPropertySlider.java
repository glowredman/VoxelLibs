...
public class VoxelPropertySlider extends VoxelProperty<IVoxelPropertyProviderFloat>
{
	...
	public void draw(IAdvancedDrawGui host, int mouseX, int mouseY)
	{
		...
		this.fontRenderer.drawStringWithShadow(this.maxText, this.xPosition + this.sliderWidth + 2, this.yPosition, 0xFFFFFF);
		...
		if (this.indicatorShow) this.drawIndicator(host);
	...
	public void moveHandle(int mouseX)
	{
		...
		if (mouseX < this.handleXPosMin || ((mouseX - this.handleXPosMin) / this.valueScale) < 0) this.value = 0;
		else if (mouseX > this.handleXPosMin + (this.handleXPosMax - this.handleXPosMin) || ((mouseX - this.handleXPosMin) / this.valueScale) > (this.valueMax - this.valueMin)) this.value = (this.valueMax - this.valueMin);
		else if (this.snap) this.value = (int)((mouseX - this.handleXPosMin) / this.valueScale);
		else this.value = (float)((mouseX - this.handleXPosMin) / this.valueScale);

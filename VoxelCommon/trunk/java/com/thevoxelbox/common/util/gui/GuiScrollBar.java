...
public class GuiScrollBar extends GuiControlEx
{
	...
	/**
	 * Mousedown offset relative to the scroll button, used for mouse drag delta calculations 
	 */
	protected int dragOffset;
	
	/**
	 * Mousedown state, track whether holding down the drag button, up button, or down button
	 */
	protected int mouseDownState = 0;
	
	/**
	 * Used to track a small delay before activating scroll on the up/down buttons
	 */
	 ...
	 */
	public GuiScrollBar(Minecraft minecraft, int controlId, int xPos, int yPos, int controlWidth, int controlHeight, int minValue, int maxValue, ScrollBarOrientation orientation)
	{
	...
	/**
	 * Set the minimum value for the control, the current value will be clamped between min and max
	 * 
	 * @param value
	 */
	...
	/**
	 * Set the maximum value for the control, the current value will be clamped between min and max.
	 * 
	 * @param value
	 */
	...
	protected boolean mouseIsOverButton(int button, int mouseX, int mouseY)
	{
		...
		{
			if (this.orientation == ScrollBarOrientation.Vertical) buttonY = this.getLargeDimension() - this.getSmallDimension();
			if (this.orientation == ScrollBarOrientation.Horizontal) buttonX = this.getLargeDimension() - this.getSmallDimension();
		}
		else if (button == 1)	// drag button 
		{
			if (this.orientation == ScrollBarOrientation.Vertical) { buttonY = this.getSmallDimension() + this.buttonPos; buttonHeight = this.scrollButtonSize; }
			if (this.orientation == ScrollBarOrientation.Horizontal) { buttonX = this.getSmallDimension() + this.buttonPos; buttonWidth = this.scrollButtonSize; }
		}
	...
	public boolean mousePressed(Minecraft minecraft, int mouseX, int mouseY)
	{
		...
		if (super.mousePressed(minecraft, mouseX, mouseY))
		{
			...
			// Counter starts now and we wait 10 seconds for mousedown effects to kick in
			...
			else if (mouseY > this.getLargeDimension() - this.getSmallDimension()) // down button
			{
				this.mouseDownState = 2;
				this.value += 4;
				this.actionPerformed = true;
			}
			else if (mouseY > this.buttonPos + this.getSmallDimension() && mouseY < this.buttonPos + this.getSmallDimension() + this.scrollButtonSize) // drag button
			{
				this.mouseDownState = 1;
				this.dragOffset = mouseY - this.buttonPos - this.getSmallDimension();
				returnValue = true;
			}
			else if (mouseY < this.buttonPos + this.getSmallDimension()) //  upper tray
			{
				this.value -= 40;
				this.actionPerformed = true;
			}
			else if (mouseY > this.buttonPos + this.getSmallDimension() + this.scrollButtonSize) // lower tray
			{
				this.value += 40;
				this.actionPerformed = true;
			}

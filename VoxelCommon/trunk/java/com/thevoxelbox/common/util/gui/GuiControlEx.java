...
public abstract class GuiControlEx extends GuiControl
{
	 ...
	 */
	public GuiControlEx(Minecraft minecraft, int controlId, int xPos, int yPos, int controlWidth, int controlHeight, String displayText)
	{
	...
	public boolean getDoubleClicked(boolean resetDoubleClicked)
	{
		...
		if (resetDoubleClicked) this.doubleClicked = false;
		return result;
	}
	...
	private void drawArrow(int x1, int y1, int x2, int y2, int width, int colour, boolean arrowHead, int arrowHeadSize)
	{
		...
		if (arrowHead && arrowHeadSize > 0)
		{
			...
			tessellator.addVertex(x2, 0 - arrowHeadSize / 2, 0);
			tessellator.addVertex(x2, arrowHeadSize / 2, 0);
			tessellator.addVertex(length, 0, 0);
			tessellator.draw();
		}
	...
	public void drawTexturedModalRectRot(int x, int y, int x2, int y2, int u, int v, int u2, int v2)
	{
		Tessellator tessellator = AbstractionLayer.getTessellator();
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(x2, y2, this.getZLevel(), (u)  * texMapScale, (v2) * texMapScale);
		tessellator.addVertexWithUV(x2, y,  this.getZLevel(), (u2) * texMapScale, (v2) * texMapScale);
		tessellator.addVertexWithUV(x,  y,  this.getZLevel(), (u2) * texMapScale, (v)  * texMapScale);
		tessellator.addVertexWithUV(x,  y2, this.getZLevel(), (u)  * texMapScale, (v)  * texMapScale);
		tessellator.draw();
	}
	...
	public void drawTexturedModalRectRot(int x, int y, int u, int v, int width, int height)
	{
		Tessellator tessellator = AbstractionLayer.getTessellator();
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(x + height, y + width, this.getZLevel(), (u)         * texMapScale, (v + height) * texMapScale);
		tessellator.addVertexWithUV(x + height, y,         this.getZLevel(), (u + width) * texMapScale, (v + height) * texMapScale);
		tessellator.addVertexWithUV(x,          y,         this.getZLevel(), (u + width) * texMapScale, (v)         * texMapScale);
		tessellator.addVertexWithUV(x,          y + width, this.getZLevel(), (u)         * texMapScale, (v)         * texMapScale);
		tessellator.draw();
	}

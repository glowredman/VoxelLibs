...
public class GuiDialogBoxConfirm<T> extends GuiDialogBox
{
	...
	/**
	 * Metadata for this confirmation, so that the calling class knows what's being confirmed!
	 */
	private T metaData;
	...
	{
		// Label
		drawCenteredString(this.fontRendererObj, this.messageText1, this.dialogX + (this.dialogWidth / 2), this.dialogY + 18, 0xFFFFAA00);
		drawCenteredString(this.fontRendererObj, this.messageText2, this.dialogX + (this.dialogWidth / 2), this.dialogY + 32, 0xFFFFAA00);
	}
	...
	{
		if (keyChar == 'y' || keyChar == 'Y') actionPerformed(this.btnOk);
		if (keyChar == 'n' || keyChar == 'N') actionPerformed(this.btnCancel);
	}
}

...
/**
 * A status message instance, this is returned from the manager to a mod to allow the mod to control
 * its own status message display
 * 
...
public class StatusMessage implements Comparable<StatusMessage>
{
	...
	/**
	 * Order the message was added, this is used to order messages which have the same priority 
	 */
	private int order = 0;
	...
	public int drawStatus(FontRenderer fontRenderer, int xPos, int yPos)
	{
		if (this.isVisible())
		{
			fontRenderer.drawStringWithShadow(String.format(MESSAGE_FORMAT, this.titleColour, this.title, this.textColour, this.text), xPos, yPos, 0xffffff);

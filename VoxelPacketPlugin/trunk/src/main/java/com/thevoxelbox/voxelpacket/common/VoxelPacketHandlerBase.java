...
public abstract class VoxelPacketHandlerBase implements IVoxelPacketHandler, IVoxelMessagePublisher
{
	...
	/**
	 * Default fallback encoder for objects, encodes serializable objects that don't have a more specific encoder
	 */
	 ...
	 */
	protected HashMap<String, VoxelMessage> messages = new HashMap<String, VoxelMessage>();
	...
	@Override
	public final void handleVoxelPacket(VoxelPacket packet, Player sender)
	{
		if (packet != null && sender != null) // && packet.isVoxelPacket)
		{
			// Dispatch happens outside of the message lock
			VoxelMessage dispatchMessage = null;
			
			// Create the packet message ID
			String messageId = sender.getUniqueId() + "[" + packet.packetMessageId + "]";
			...
			synchronized (this.messagesLock)
			{
				...
				catch (InvalidReplicationException e)
				{
					Bukkit.getServer().getPlayer(sender.getName()).kickPlayer("Replicating entities from client to server is not allowed");
					this.messages.remove(messageId);
				}
			}
